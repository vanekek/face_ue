...
def copy_template_images(data_path, test_template_path, template_id, image_paths, template_ids):
...
@hydra.main(
    config_path=str(
        Path(__file__).resolve().parents[1] / "configs/hard_samples"
    ),
...
def main(cfg):
    ...
    for test_dataset in test_datasets:
        ...
        probe_unique_templates = np.unique(tt.test_dataset.probe_templates, return_index=False)
        ...
        for gallery_name, (g_templates_feature, g_template_unc, g_unique_ids) in zip(
            ...
            for method in methods:
                ...
                predictions[method.pretty_name] = (predicted_id, was_rejected, predicted_unc)
            ...
            for vMF_unc, scf_unc, false_ident_id in zip(vMF_unc_score[is_seen][false_ident], scf_unc_score[is_seen][false_ident], probe_unique_templates[is_seen][false_ident]):
                probe_template_path = (
                    out_image_dir / 'false_ident'
                    / f"probe_id-{str(false_ident_id)}_scf-unc-{scf_unc}_vMF-unc-{vMF_unc}"
                )
                copy_template_images(data_path, probe_template_path, false_ident_id, image_paths, template_ids)
