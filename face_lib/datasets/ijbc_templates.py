...
class IJBCTemplates:
    ...
    def init_proto(self, proto_folder):
        ...
        self._pairs = pd.read_csv(
            self.proto_folder / "cropped_matches.csv", header=None
        ).to_numpy()
        # self._pairs = pd.read_csv(self.proto_folder / 'short_matches.csv', header=None).to_numpy()

        enroll_dict = build_templates(
            enroll_path, self.feature_dict, self.uncertainty_dict
        )
        self.templates_dict.update(enroll_dict)
        self._enroll_templates = enroll_dict.values()

        verify_dict = build_templates(
            verif_path, self.feature_dict, self.uncertainty_dict
        )
    ...
    def get_features_uncertainties_labels(self):
        ...
        labels1 = np.array(
            [
                ...
                for t in tqdm(self._pairs[:, 0], desc="Labels")
            ]
        )
        labels2 = np.array(
