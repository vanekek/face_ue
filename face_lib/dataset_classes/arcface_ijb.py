...
class IJB_aligned_images(Dataset):
    def __init__(self, dataset_path: str, dataset_name: str, norm_image: bool = False) -> None:
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        if self.norm_image:
            img = (img - np.array([0.5, 0.5, 0.5])[np.newaxis, np.newaxis,:]) /  np.array([0.5, 0.5, 0.5])[np.newaxis, np.newaxis,:]
