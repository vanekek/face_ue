...
class PartialFC(Module):
    ...
    def __init__(
        ...
        self.weight_name = os.path.join(
            self.source, "rank_{}_softmax_weight.pt".format(self.rank)
        )
        self.weight_mom_name = os.path.join(
            self.source, "rank_{}_softmax_weight_mom.pt".format(self.rank)
        )
    ...
    def forward_backward(self, label, features, optimizer, scale=None):
        ...
        if scale is None:
            ...
            # feature gradient all-reduce
            dist.reduce_scatter(
                s_grad, list(total_scale.grad.chunk(self.world_size, dim=0))
            )
