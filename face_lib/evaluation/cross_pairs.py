...
def generate_val_test_split(dataframe, dataset_dir):
    ...
    def suitable(row, identities):
        return (
            cut_name(row.photo_1) in identities and cut_name(row.photo_2) in identities
        )

    test_df = dataframe[
        dataframe.apply(lambda row: suitable(row, test_identities), axis=1)
    ]
    val_df = dataframe[
        dataframe.apply(lambda row: suitable(row, val_identities), axis=1)
    ]
