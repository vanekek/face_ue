...
def get_required_models(checkpoint, args, model_args, device=torch.device("cpu")):
    ...
    if args.uncertainty_strategy == "head" or (
        args.uncertainty_strategy == "classifier" and "head" in model_args
    ):
...
def get_distance_uncertainty_funcs(
    ...
    if distance_name == "classifier":
        distance_func = classifier_to_distance_wrapper(classifier, device=device)
    else:
        distance_func = name_to_distance_func[distance_name]

    if uncertainty_name == "classifier":
        uncertainty_func = classifier_to_uncertainty_wrapper(classifier, device=device)
    else:
        uncertainty_func = name_to_uncertainty_func[uncertainty_name]

    if distance_name in [
        "biased-cosine",
        "scale-mul-biased-cosine",
        "scale-harmonic-biased-cosine",
        "scale-sqrt-mul-biased-cosine",
        "scale-sqrt-harmonic-biased-cosine",
        "pfe-mul-biased-cosine",
        "pfe-harmonic-biased-cosine",
        "pfe-sqrt-mul-biased-cosine",
        "pfe-sqrt-harmonic-biased-cosine",
    ]:
        ...
        new_distance_func = lambda x1, x2, unc1, unc2: distance_func(
            x1, x2, unc1, unc2, bias=val_statistics["mean_cos"]
        )  # TODO: Fix it
    ...
    if distaces_batch_size:
        new_distance_func = split_wrapper(
            new_distance_func, batch_size=distaces_batch_size
        )
        new_uncertainty_func = split_wrapper(
            new_uncertainty_func, batch_size=distaces_batch_size
        )
...
def find_gaussian_optimal_threshold(similarities, labels):
    ...
    p_2 = (
        (pos_mean**2) / (2 * pos_var)
        - (neg_mean**2) / (2 * neg_var)
        + 0.5 * np.log(neg_var / pos_var)
    )
...
def get_mean_classes_centres(similarities, label_vec):
    return 0.5 * (
        similarities[label_vec].mean(axis=0) + similarities[~label_vec].mean(axis=0)
    )
...
def extract_statistics(data):
    ...
    print(
        f"Mean thres : {mean_cosine} best_f1 : {best_f1_thres} optimal_gauss : {best_gauss_thres}"
    )
