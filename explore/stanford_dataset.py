...
def make_small(data_dir, small_dir):
    df = pd.read_csv(
        data_dir / "Ebay_info.txt", delim_whitespace=True, index_col="image_id"
    )
...
def ffcv_loader_by_df(
    df, base_dir, write_path, random_order=False, batch_size=100, augment=False
):
    ...
    writer = DatasetWriter(
        write_path, {"image": RGBImageField(max_resolution=32), "label": IntField()}
    )
    ...
    #     ])
    image_pipeline.extend([transforms.Normalize(127.0, 50.0)])
...
def get_loaders(base_dir, super_classes=False, split=(20_000, 12_000), batch_size=128):
    ...
    df = pd.read_csv(
        data_dir / "Ebay_train.txt", delim_whitespace=True, index_col="image_id"
    )
