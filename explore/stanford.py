...
from explore.stanford_trainers import (
    TripletsTrainer,
    CrossEntropyTrainer,
    ArcFaceTrainer,
)
...
def main():
    ...
    parser.add_argument(
        "--base_dir",
        type=str,
        default="/home/kirill/data/stanford",
        help="Directory with dataset and models",
    )
    parser.add_argument(
        "--method",
        default="classification",
        choices=["classification", "triplets", "arcface", "scale"],
    )
    ...
    if args.method == "classification":
        ...
        trainer = ArcFaceTrainer(
            model, embedding_size=embedding_size, num_classes=num_classes, epochs=50
        )
        trainer.train(train_loader, val_loader)
        ...
        model = get_confidence_model(
            "resnet9_scale", num_classes, checkpoint_dir / "resnet9_arcface.pth"
        )
