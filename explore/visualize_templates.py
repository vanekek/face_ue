...
parser.add_argument(
    "--test_folder", default="/gpfs/gpfs0/k.fedyanin/space/figures/test"
)
...
methods = [
    ...
    {"name": "magface", "functions": ("mean", "cosine", "mean"), "label": "MagFace"},
    {
        ...
        "label": "ScaleFace (ours)",
    },
    {
        "name": "scale",
        "functions": ("mean", "scale-sqrt-harmonic-biased-cosine", "harmonic-harmonic"),
        "label": r"$\mu$-ScaleFace (ours)",
    },
    # {
...
def plot_TAR_FAR_different_methods(
    ...
    plots_indices = {FAR: idx for idx, FAR in enumerate(FARs)}
    ...
    for key, table in results.items():
        for FAR, TARs in table.items():
            ...
            axes[plots_indices[FAR]].plot(
                rejected_portions, TARs, label=label, marker=" ", linewidth=3, alpha=0.8
            )
